...
public class Question11 {
    public static void main(String[] args) {
        ...
        float principle_amt, rate_percentage, rate_amt, time, final_amt = 0;

        System.out.println("Enter principle amount: ");
        principle_amt = input.nextFloat();

        System.out.println("Enter rate percentage: ");
        rate_percentage = input.nextFloat();

        rate_amt = principle_amt * rate_percentage;
        ...
        while (true) {
            ...
            if (op == '1' || op == '2' || op == '3' || op == '4' || op == '5' || op == '6') {
                ...
                if (op == '1') {
                    final_amt = (float) (principle_amt * (Math.pow((1 + rate_amt), time)));
                } else if (op == '2') {
                    final_amt = (float) (principle_amt * (Math.pow((1 + (rate_amt / 2)), (2 * time))));
                } else if (op == '3') {
                    final_amt = (float) (principle_amt * (Math.pow((1 + (rate_amt / 4)), (4 * time))));
                } else if (op == '4') {
                    final_amt = (float) (principle_amt * (Math.pow((1 + (rate_amt / 12)), (12 * time))));
                } else if (op == '5') {
                    final_amt = (float) (principle_amt * (Math.pow((1 + (rate_amt / 52)), (52 * time))));
                } else if (op == '6') {
                    final_amt = (float) (principle_amt * (Math.pow((1 + (rate_amt / 365)), (365 * time))));
                }
            } else if (op == 'x' || op == 'X') {
                break;
            } else {
